// Task
//
// Coding in function sortIt. function accept 1 parameters arr, it's a number array.
// Your task is to sort the array according to the specified conditions, and return
// a new array (should not modify the original array).
//
// conditions1:
// According to the number of elements(in ascending order) for example:
//
//   sortIt([1,1,1,2,2,3]) should return [3,2,2,1,1,1]
//   because [1,1,1,2,2,3] has one 3, two 2 and three 1
//
// conditions2:
// If the same number of elements, according to the number values(in descending order) for example:
//
//   sortIt([1,1,1,2,2,2,3,3,3]) should return [3,3,3,2,2,2,1,1,1]
//   because number of 3,2 and 1 both are three, then according to 3>2>1
//
// Comprehensive two conditions should be like this:
//
//   sortIt([1,2,3,4,4,5,5,6,6]) should return [3,2,1,6,6,5,5,4,4]

var compare = (a, b) => {
  switch (true) {
  case a.length < b.length:
    return -1;
  case a.length > b.length:
    return 1;
  case a[0] > b[0]:
    return -1;
  case a[0] < b[0]:
    return 1;
  }
}

var sortIt = (arr) => {
  var counters = {};
  var collection = [];

  arr.forEach(
    (n) => {
      counters[n] = counters[n] ? counters[n].concat(n) : [n];
    }
  );

  for (var key in counters) {
    var value = counters[key];
    collection.push(value);
  }

  return [].concat.apply([], collection.sort(compare));
}

console.log(sortIt([1, 1, 1, 2, 2, 3]));
console.log(sortIt([1, 1, 1, 2, 2, 2, 3, 3, 3]));
console.log(sortIt([1, 2, 3, 4, 4, 5, 5, 6, 6]));
